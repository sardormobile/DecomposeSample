...
class RootComponent(
            ...
            Configuration.ScreenA -> Child.ScreenA(
                ScreenAComponent(
                    ...
                    onNavigateToScreenB = {text ->
                        navigation.pushNew(Configuration.ScreenB(text = text))
                    })
            )
    ...
    sealed class Child {
        data class ScreenA(val componentAScreen: ScreenAComponent): Child()
        data class ScreenB(val componentBScreen: ScreenBComponent): Child()
    }
    @Serializable
